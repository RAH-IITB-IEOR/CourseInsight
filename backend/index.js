...
app.get("/course/all", async (req, res) => {
  try {
    const courses = await db.collection("Course").find(
      {},
      { projection: { _id: 0, courseID: 1, courseName: 1 } }
    ).toArray();
...
app.get("/course/:id", async (req, res) => {
  try {
    ...
    if (course.instructorID) {
      const instructor = await db.collection("Instructor").findOne(
...
app.post("/comment/:courseID", verifyToken, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Comment submitted and pending verification" });
  } catch (error) {
...
app.get("/comment/unverified", verifyToken, isAdmin, async (req, res) => {
  try {
    const unverifiedComments = await db.collection("Comment")
